...
@Component({
  ...
  templateUrl: './doberman.html',
  styleUrl: './doberman.css',
})
export class Doberman implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('doberman').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'doberman');
        } else {
