...
@Component({
  ...
  templateUrl: './corgi.html',
  styleUrl: './corgi.css',
})
export class Corgi implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('corgi').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'corgi');
        } else {
