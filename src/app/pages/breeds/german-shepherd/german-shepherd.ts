...
@Component({
  ...
  templateUrl: './german-shepherd.html',
  styleUrl: './german-shepherd.css'
})
export class GermanShepherd implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('german shepherd').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'german shepherd');
        } else {
