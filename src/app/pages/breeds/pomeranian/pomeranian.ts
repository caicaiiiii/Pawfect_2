...
@Component({
  ...
  templateUrl: './pomeranian.html',
  styleUrl: './pomeranian.css',
})
export class Pomeranian implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('pomeranian').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'pomeranian');
        } else {
