...
@Component({
  ...
  templateUrl: './french-bulldog.html',
  styleUrl: './french-bulldog.css',
})
export class FrenchBulldog implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('french bulldog').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'french bulldog');
        } else {
