...
@Component({
  ...
  templateUrl: './siberian-husky.html',
  styleUrl: './siberian-husky.css',
})
export class SiberianHusky implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('siberian husky').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'siberian husky');
        } else {
