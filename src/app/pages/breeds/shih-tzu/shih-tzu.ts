...
@Component({
  ...
  templateUrl: './shih-tzu.html',
  styleUrl: './shih-tzu.css',
})
export class ShihTzu implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('shih tzu').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'shih tzu');
        } else {
