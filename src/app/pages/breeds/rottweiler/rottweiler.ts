...
@Component({
  ...
  templateUrl: './rottweiler.html',
  styleUrl: './rottweiler.css',
})
export class Rottweiler implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('rottweiler').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'rottweiler');
        } else {
