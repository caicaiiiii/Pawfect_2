...
@Component({
  ...
  templateUrl: './border-collie.html',
  styleUrl: './border-collie.css',
})
export class BorderCollie implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('border collie').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'border collie');
        } else {
