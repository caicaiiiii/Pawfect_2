...
@Component({
  ...
  templateUrl: './poodle.html',
  styleUrl: './poodle.css',
})
export class Poodle implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('poodle').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'poodle');
        } else {
