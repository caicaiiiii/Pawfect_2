...
@Component({
  ...
  templateUrl: './maltese.html',
  styleUrl: './maltese.css',
})
export class Maltese implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('maltese').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'maltese');
        } else {
