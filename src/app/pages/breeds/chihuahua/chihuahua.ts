...
@Component({
  ...
  templateUrl: './chihuahua.html',
  styleUrl: './chihuahua.css',
})
export class Chihuahua implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('chihuahua').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'chihuahua');
        } else {
