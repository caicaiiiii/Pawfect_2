...
@Component({
  ...
  templateUrl: './golden-retriever.html',
  styleUrl: './golden-retriever.css'
})
export class GoldenRetriever implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('golden retriever').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'golden retriever');
        } else {
