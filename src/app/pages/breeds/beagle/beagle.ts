...
@Component({
  ...
  templateUrl: './beagle.html',
  styleUrl: './beagle.css'
})
export class Beagle implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('beagle').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'beagle');
        } else {
