...
@Component({
  ...
  templateUrl: './dachshund.html',
  styleUrl: './dachshund.css',
})
export class Dachshund implements OnInit {
  ...
  loadBreed(): void {
    ...
    this.dogNinjaService.getDogByName('dachshund').subscribe({
      next: (response: DogNinjaBreed[]) => {
        console.log('API response:', response);

        if (response && response.length > 0) {
          this.breed = this.findBestMatch(response, 'dachshund');
        } else {
